<?php
	...
	for ($i=0; $i<$LISTA_EVALUACION_PASADA[max_res]; $i++){
	  ...
	  $sql ="SELECT id_fam FROM ENCUESTA WHERE id_encuesta_ls='".$LISTA_EVALUACION_PASADA["Enc"]["id_encuesta_ls"][$i]."'";
	  $atts = array("id_fam");
	  $FAMILIA_CARGOS= obtenerDatos($sql, $conexion, $atts, "Fam");
	  $LISTA_EVALUACION_PASADA["Enc"]["id_fam"][$i]=$FAMILIA_CARGOS["Fam"]["id_fam"][0];//Familia de cargos asociada a la encuesta
      ...
      if (isset($_GET['token_ls']) && isset($_GET['id_encuesta_ls'])) {
		...
		// La respuesta final se limpia el "
		if($i==count($RESPUESTAS)-1){
			$tipo_respuesta_i = substr($RESPUESTAS[$i],1,2);
