<?php
    session_start();
    $Legend = "Historial de Cargos";
    include "lib/cVerCargosPersona.php";
    include "vHeader.php";
    $all = true;
?>   
...
<?php
  ...
  if ($LISTA_CAR['max_res']==0){
      if(!isAdmin())
        echo "<br><br><br><br><br><br><p class='text-center text-info'>Hasta el momento esta persona no tiene cargos registrados.</p><br><br><br><br><br><br>";
      else
        echo "<br><br><br><br><br><br><p class='text-center text-info'>Hasta el momento esta persona no tiene cargos registrados.</p><br><br><br><br><br><br>";
  }else{
  ?>
  ...
  <tfoot>
    <tr>
      ...
      <th class="lsmallT"><small>Observaciones</small></th>
          ...
          <?
            for ($i=0;$i<$LISTA_CAR['max_res'];$i++){
          ?>
          <tr class="<?php echo $color_tabla; ?>" >
          <td class="center lsmallT" nowrap><small><a href='vCargo.php?view&id=<? echo $LISTA_CAR['Car']['id_car'][$i];?>'> <? echo $CAR_ID[$LISTA_CAR['Car']['id_car'][$i]];?></a> <? if ($LISTA_CAR['Car']['actual'][$i] == 't') echo " (actual)"; ?></small></td>
          <td class="center lsmallT"><small><? echo $LISTA_CAR['Car']['fecha_ini'][$i];?></small></td>
          <td class='center lsmallT'><small><? echo $LISTA_CAR['Car']['fecha_fin'][$i];?></small></td>
          <td class='center lsmallT'><small><? echo $LISTA_CAR['Car']['observacion'][$i];?></small></td>
          </tr>
          <? } ?>     
        </tbody>
</table>
